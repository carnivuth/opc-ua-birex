...
// Security flags
var securityPolicy = flag.String("security-policy", "", "Security policy (e.g., Aes128Sha256RsaOaep, Basic256Sha256, None)")
...
func main() {
	...
	if err != nil {
		log.Fatalf("Error creating OPC UA client: %v", err)
	}
...
func getClient(endpoint *string) (*opcua.Client, error) {
	...
	// Se sono specificati parametri di sicurezza, configurali
	if *securityPolicy != "" && *securityMode != "" && *certFile != "" && *keyFile != "" {
		log.Printf("Configuring security: Policy=%s, Mode=%s", *securityPolicy, *securityMode)
		...
		pk, err := parsePrivateKeyPEM(keyData)
		if err != nil {
			return nil, fmt.Errorf("failed to parse private key: %v", err)
		}

		// Converti la policy string al tipo corretto
		policy := securityPolicyURI(*securityPolicy)

		// Converti il security mode string al tipo corretto
		mode := securityModeFromString(*securityMode)
		...
		log.Print("Security configured successfully")
	} else if *securityPolicy != "" || *securityMode != "" || *certFile != "" || *keyFile != "" {
		return nil, fmt.Errorf("if using security, all of -security-policy, -security-mode, -cert, and -key must be specified")
	}
...
// Parse della chiave privata da formato PEM usando le librerie standard Go
...
// Mappa la stringa della policy al formato URI richiesto da gopcua
func securityPolicyURI(policy string) string {
	...
	default:
		return policy // Assume it's already a full URI
	}
}

// Converti la stringa del security mode al tipo MessageSecurityMode
